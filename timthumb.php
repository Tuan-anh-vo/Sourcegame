...
if(! defined('FILE_CACHE_DIRECTORY') ) 		define ('FILE_CACHE_DIRECTORY', './cache_images/');				// Directory where images are cached. Left blank it will use the system temporary directory (which is better for security)
...
if(! defined('WATERMARK_OVERLAY_IMAGE') )	define( 'WATERMARK_OVERLAY_IMAGE', '../../images/watermark.png');
...
class timthumb {
	...
	protected function processImageAndWriteToCache($localImage){
		...
		if ($watermark == 1) {
			...
			$wmdim = $this->resize_dimensions($new_width/3,$new_height/3,$overlay_width,$overlay_height);
			
			$overlay_width =  $wmdim['width'];
			$overlay_height = $wmdim['height'];
			

			$new_image = imagecreatetruecolor ( $overlay_width, $overlay_height ); // new wigth and height
			imagealphablending($new_image , false);
			imagesavealpha($new_image , true);
			
			imagecopyresampled ( $new_image, $overlay_gd_image, 0, 0, 0, 0, $overlay_width, $overlay_height, imagesx ( $overlay_gd_image ), imagesy ( $overlay_gd_image ) );
			
// 			$watermark_positionx = ($new_width/2 - $overlay_width/2);
// 			$watermark_positiony = ($new_height/2 - $overlay_height/2);

			$dest_x         = $new_width - $overlay_width - 5;
			$dest_y         = $new_height - $overlay_height - 5;
				
			imagealphablending($canvas, true );
			imagecopy( $canvas, $new_image,$dest_x, $dest_y, 0, 0, $overlay_width, $overlay_height);
			imagealphablending($canvas, false );
			imagesavealpha($canvas , true);
		}
	...
	protected function resize_dimensions($goal_width,$goal_height,$width,$height) {
