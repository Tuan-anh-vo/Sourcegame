...
class Notification extends MX_Controller {
    ...
    public function admincp_update($id=0){
        ...
        $data = array(
            ...
            'module'=>$this->module,
            'id'=>$id,
            'servers'=>$this->model->fetch('*','cli_servers') 
        );
    ...
    function get_notice(){
        ...
        $query=$this->db->query("SELECT * FROM  cli_notification WHERE server_id LIKE '%-".$server_id."-%'
                                AND status=1
                                 AND DATE(`start_time`)<='".date('Y-m-d')."'  AND DATE(`end_time`)>='".date('Y-m-d')."'
        ");
        $result=$query->row();
        if(!isset($result->id)){
            return false;
        }
        echo "<div class='wapnoti'>".$result->content."</div>";die;
    }
